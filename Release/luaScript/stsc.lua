...
function Parse(name,data)
  ...
  for i = 1,entry_count do
    ...
    index = index + 4
    result = result.."       first_chunk: (0x"..HEX(first_chunk,4)..") "..Integer(first_chunk,4).."\r\n"

    samples_per_chunk = string.sub(data,index + 1,index + 4)
    index = index + 4
    result = result.."       samples_per_chunk: (0x"..HEX(samples_per_chunk,4)..") "..Integer(samples_per_chunk,4).."\r\n"

    sample_description_index = string.sub(data,index + 1,index + 4)
    index = index + 4
    result = result.."       sample_description_index: (0x"..HEX(sample_description_index,4)..") "..Integer(sample_description_index,4).."\r\n"
  end
