...
function Parse(name,data)
  ...
  for i = 1,entry_count do
    first_chunk = string.sub(data,index + 1,index + 4)
    index = index + 4
    table.insert(result , "       first_chunk: (0x"..HEX(first_chunk,4)..") "..Integer(first_chunk,4))

    samples_per_chunk = string.sub(data,index + 1,index + 4)
    index = index + 4
    table.insert(result , "       samples_per_chunk: (0x"..HEX(samples_per_chunk,4)..") "..Integer(samples_per_chunk,4))

    sample_description_index = string.sub(data,index + 1,index + 4)
    index = index + 4
    table.insert(result , "       sample_description_index: (0x"..HEX(sample_description_index,4)..") "..Integer(sample_description_index,4))
  end
  -- -- result = result.."       index:"..index.." size:"..size.."\r\n"
  table.insert(result , "----------------------------------------------------------------")
  table.insert(result , "Ω≤Ω‚µÿ÷∑:")
